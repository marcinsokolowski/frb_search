     ...
     for( int y=0; y < y_size; y++){
        for( int x=0; x < x_size; x++){
            ...
            if( pFits ){
               ...
               mean_spectrum.SaveToFile( szOutMeanSpectrum, NULL, &rms_spectrum ); // WARNING : this line used to crash the code if the output directory did not exit, fixed now inside CBgArray::SaveToFile function
